import { Admin, Patient, User } from "@/Models/UserModel/UserModel"
...
export async function POST(request: Request){
    try{
        ...
        if(existingPatient){
            ...
            const createdPatient = await Admin.create({ name, lastname, email, password: hashedPassword, phone, role: "admin" })
            if(createdPatient){
