        ...
        <main className="w-screen h-screen flex items-center justify-center px-8 py-4">
            <div className="w-[32rem] px-8 py-4 rounded-md shadow-lg border-sickness-border border bg-white flex flex-col gap-4 items-center">
                ...
                <div className="w-full">
                    ...
                    <input type="text" value={LoginData.email} onChange={(e)=>{dispatch(setLoginFormData({ name: "email", value: e.target.value  })); setLoginErrors(prevErrors => ({...prevErrors, email: ""}))}} className="outline-none border focus:border-sickness-primary text-sickness-gray text- pl-2 py-2 rounded-md w-full border-sickness-border" placeholder="example@gmail.com" />
                ...
                <div className="w-full mt-4">
                    ...
                    <input type="password" value={LoginData.password} onChange={(e)=>{dispatch(setLoginFormData({ name: "password", value: e.target.value  })); setLoginErrors(prevErrors => ({...prevErrors, password: ""}))}} className="outline-none border focus:border-sickness-primary text-sickness-gray text- pl-2 py-2 rounded-md w-full border-sickness-border" placeholder="●●●●●●●●" />
                    <p className="text-sm text-red-500 break-words"> {loginErrors.password} </p>
                </div>
                <button className={`mt-4 w-full rounded-md text-white ${requestLoading ? "bg-sickness-primary/70" : "bg-sickness-primary hover:bg-sickness-primaryText/70 active:bg-sickness-primaryText"} transition delay-75 duration-100 h-12 font-semibold flex justify-center items-center gap-1`} onClick={handleLogin} disabled={requestLoading}> Login { requestLoading ? <div className="small-white-loader" /> : <LogInIcon /> } </button>
